...
def listar_itens_inscritos():
    ...
    meus_itens_inscritos = "https://steamcommunity.com/id/damacenikos/myworkshopfiles/?appid=255710" \
                           "&browsesort=mysubscriptions" \
                           "&browsefilter=mysubscriptions" \
                           "&numperpage=30"
    abrir_pagina(meus_itens_inscritos)
    proxima_pagina = True

    with open("Meus itens da oficina.csv", "w", encoding="utf-8", newline="") as f:
        thewriter = csv.writer(f)
        thewriter.writerow(["IMAGEM", "NOME", "LINK"])
        while proxima_pagina:
            ...
            for item in itens_inscritos:
                ...
                link = item.find_element_by_css_selector("div.workshopItemSubscriptionDetails > a").get_attribute("href")
